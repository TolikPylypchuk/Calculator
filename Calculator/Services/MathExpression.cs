...
/// </summary>
/// <remarks>
/// The expressions used as arguments in the methods in this class should be built by the ExpressionBuilder class.
/// </remarks>
        ...
        foreach (string token in infixForm.Trim().Split(Delimiter))
        {
            ...
            } else
            {
                switch (token)
                {
                    ...
                    case "*":
                    case "/":
                        while (stack.Count != 0 && "×/^sincosctg√".Contains(stack.Peek(), StringComparison.Ordinal))
                        {
                            result += stack.Pop();
                            result += Delimiter;
                        }
